...
def _wrap_class(wrapper, user_class):
    user_class.__cache_instances_factory__ = wrapper

    # If any of `__reduce__`, `__reduce_ex__`, `__getnewargs__`,
    # and `__getnewargs_ex__` is defined, retain it and let the process of
    # unpickling use it.
    #
    # Force to use execute `__new__` when unpickling to support the classes
    # which can be initiated by one or more arguments.
    if getattr(user_class, '__reduce__', None) is type.__reduce__ and \
        getattr(user_class, '__reduce_ex__', None) is type.__reduce_ex__ and \
        not hasattr(user_class, '__getnewargs_ex__') and \
        not hasattr(user_class, '__getnewargs__'):
        def __reduce_ex__(self, protocol):
            print(self.__newkwargs__)
            return (
                self.__class__.__new__, 
                (self.__class__, self.__newargs__, self.__newkwargs__))
        user_class.__reduce_ex__ = __reduce_ex__
    ...
    # XXX: The names of the extra attributes of the wrapper may be used in the
    #   `user_class`, should they be assigned forcely?
    # update_wrapper(
    #     user_class,
    #     wrapper,
    #     (
    #         'info', 'clear', 'load', 'dump', 'archive', 'archived',
    #         'key', 'lookup', '__cache__', '__mask__', '__map__'
    #     ), {})
    return user_class  
...
class no_cache(object):
    ...
    def __call__(self, user_function):
       ...
       #wrapper._queue = None  #XXX
        ...
        if not isclass(user_function):
            return wrapper
        else:
            return _wrap_class(wrapper, user_function) 
        assert False
