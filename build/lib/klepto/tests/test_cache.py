...
def test_class():
    # classes = [Parent, ParentNew, Child, ChildNew, ChildBoth]
    # for Kls in classes:
    #     assert Kls(1).val
        #  == Kls(1).val
        # assert Kls.__cache_instances_factory__.info().hit == 1
        # assert Kls.__cache_instances_factory__.info().load == 1
    _cleanup()

def _cleanup():
    import pox
    try: pox.rmtree('cache_class')
    except: pass
