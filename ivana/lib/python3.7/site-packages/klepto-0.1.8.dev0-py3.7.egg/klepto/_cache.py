...
def _keygen_inputs(func, ignore):
    """A hack for making the caches can apply to classes.

    XXX: Is there any better way to achive the goal under the current
        structure?
    """
    if inspect.isclass(func):
        new_func = None
        if '__new__' in func.__dict__:
            new_func = copy(func.__new__)
            new_func.__name__ = '__init__'
        elif '__init__' in func.__dict__:
            new_func = func.__init__
        if new_func is not None:
            func = new_func
            _args = _getargspec(func).args
            if _args:
                ignore = (*ignore, _args[0])
    return func, ignore
...
def _wrap_class(wrapper, user_class):
    ...
    if getattr(user_class, '__reduce__') is type.__reduce__ and \
        getattr(user_class, '__reduce_ex__') is type.__reduce_ex__ and \
    ...
    # fix inheritance mechanism
    if '__init' in user_class.__dict__ or \
        not hasattr(user_class, '__original_init__'):
        user_class.__original_init__ = original_init = user_class.__init__
    else:
        # give the subclass the original `__init__`
        original_init = user_class.__original_init__
    ...
    def wrap_init(self, *args, **kwargs):
        if '__to_original_m061p3__' in kwargs:
            ...
            print('before go original init')
            original_init(self, *args, **kwargs)
            print('after original init')
        else:
            print('else')
            # original_init(self, *args, **kwargs)
...
class no_cache(object):
    ...
    def __call__(self, user_function):
       ...
       #lock = RLock()                  # linkedlist updates aren't threadsafe
        ...
        _user_function, _ignore = _keygen_inputs(user_function, ignore)

        def wrapper(*args, **kwds):
            _args, _kwds = rounded_args(*args, **kwds)
            # `_user_function` is changed in `_keygen_inputs()`
            if _user_function is not user_function:
                # Magic! Explain:
                ...
                # True
                _args = (None, *_args)
            _args, _kwds = _keygen(_user_function, _ignore, *_args, **_kwds)
